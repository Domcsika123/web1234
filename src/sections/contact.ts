  ...
  <section class="section" id="contact">
    <div class="container">
      ...
      <div class="contact-grid" data-reveal>
        ...
        <form id="contactForm" class="contact-form" data-reveal action="https://formspree.io/f/xojednlb" method="POST">
          <h3>${t("contact.formTitle")}</h3>
          
          <div class="form-group">
            <label for="c_name">${t("contact.form.name")}</label>
            <input id="c_name" name="name" type="text" required placeholder="${t("contact.form.placeholders.name")}" />
          </div>

          <div class="form-group">
            <label for="c_email">${t("contact.form.email")}</label>
            <input id="c_email" name="email" type="email" required placeholder="${t("contact.form.placeholders.email")}" />
          </div>

          <div class="form-group">
            <label for="c_phone">${t("contact.form.phone")}</label>
            <input id="c_phone" name="phone" type="tel" placeholder="${t("contact.form.placeholders.phone")}" />
          </div>

          <div class="form-group">
            <label for="c_type">${t("contact.form.projectType")}</label>
            <select id="c_type" name="projectType" required>
          ...
          <div class="form-group">
            ...
            <textarea id="c_msg" name="message" required placeholder="${t("contact.form.placeholders.message")}"></textarea>
          </div>
          ...
          <div style="font-size: 12px; color: var(--muted); margin-top: 12px;">
            <input type="checkbox" id="c_agree" name="privacy" required style="margin-right: 6px;" />
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast("Hiba", "Az űrlap küldése nem sikerült. Próbáld újra.");
      }
    } catch {
      toast("Hiba", "Nem sikerült elküldeni. Ellenőrizd a kapcsolatot.");
    }
  });
}
